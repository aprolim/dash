...
// ============================================
// Convertir bloques a HTML - Genera MARCADORES en lugar de HTML complejo
const convertirBloquesAHTML = (blocks: any[]) => {
  ...
  for (const block of blocks) {
    if (block.type === 'paragraph') {
      ...
      // 🔥 Generar marcador [[CITA:autor|cargo|texto]]
      const author = block.author || 'Senado de Bolivia'
      const role = block.role || 'Cámara de Senadores'
      const content = block.content || ''
      html += `<p>[[CITA:${author}|${role}|${content}]]</p>`
    } else if (block.type === 'video') {
      // 🔥 Generar marcador [[VIDEO:url|título|caption]]
      const url = block.url || ''
...
// ============================================
// CONVERTIR HTML A BLOQUES - CON DETECCIÓN DE MARCADORES
// ============================================
...
const saveNews = async () => {
  ...
  console.log('📦 [edit] Bloques a guardar:', JSON.stringify(bloques, null, 2))
